// app/components/CreateWidgetOrPageModal.jsx
...
const WIDGET_TYPES = [
  {
    id: "carousel",
    name: "Carousel",
    image: "https://cdn.shopify.com/s/files/1/0580/9258/5030/files/quinn_m28baghh95hofagj8nlpeppc.png?v=1760445583",
  },
  {
    id: "stories",
    name: "Stories",
    image: "https://cdn.shopify.com/s/files/1/0580/9258/5030/files/quinn_b5w8kph5ndijcw77zufk31be.png?v=1760445582",
  },
  {
    id: "floating",
    name: "Floating",
    image: "https://cdn.shopify.com/s/files/1/0580/9258/5030/files/quinn_ng19ck1x9ipqa5v01ei6zeb2.png?v=1760445582",
  },
];

export default function CreateWidgetOrPageModal({
  open,
  onClose,
  videos,
  shopifyPages,
  ...
  const [selectedWidget, setSelectedWidget] = useState(preselectedWidgetType || "");
  const [selectedPage, setSelectedPage] = useState("");
  const [selectedVideos, setSelectedVideos] = useState([]);
  const [pageSearchValue, setPageSearchValue] = useState("");
  const [videoSearchValue, setVideoSearchValue] = useState("");

  const isWidgetMode = mode === "createWidget";

  // Auto-skip step 1 if widget mode
  useEffect(() => {
    if (isWidgetMode && preselectedWidgetType) {
      setSelectedWidget(preselectedWidgetType);
      setCurrentStep(2);
    }
  }, [isWidgetMode, preselectedWidgetType]);
  ...
  const handleCreate = () => {
    ...
    if (isWidgetMode && videoPageId) {
      form.append("actionType", "createWidget");
      form.append("widgetType", preselectedWidgetType);
      form.append("videoIds", JSON.stringify(selectedVideos));
      form.append("videoPageId", videoPageId);
    } else {
      const pageData = shopifyPages.find((p) => p.id === selectedPage);
      form.append("actionType", "createVideoPage");
      form.append("pageName", pageData?.title || "New Page");
      form.append("widgetType", selectedWidget);
      form.append("pagePath", pageData?.handle || "/");
      form.append("videoIds", JSON.stringify(selectedVideos));
    }
  ...
  const filteredVideos = videos.filter(
    (v) =>
      !videoSearchValue ||
      v.title?.toLowerCase().includes(videoSearchValue.toLowerCase())
  );

  const filteredPages = shopifyPages.filter((p) =>
    p.title.toLowerCase().includes(pageSearchValue.toLowerCase())
  );
